...
```{r depth bar plot}
depthplot <- trophic_expand %>% 
  replace_with_na_all(condition = ~.x %in% na_strings) %>%
  subset(!is.na(depth_max_m)) #%>% 
depthplot$depth_max_m <- as.numeric(as.character(depthplot$depth_max_m))
depthplot %>% 
  ggplot(aes(x=trophic_status, y = depth_max_m)) + geom_boxplot() +
...
boxplotdata2 <- newdata %>%  #changed 0 and 1 for phyto_wl_increase :( after poster print :(
